...
const EXAMPLE_PATH = path.join(PLUGIN_ROOT, 'agentwright.example.json');
...
describe('agentwright config-init script', () => {
  let projectDir;

  beforeEach(() => {
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-cfg-'));
  });
  ...
  it('writes .claude/agentwright.json when no config exists', () => {
    const result = runScript({ projectDir });

    assert.equal(result.exitCode, 0);
    const target = path.join(projectDir, '.claude', 'agentwright.json');
  ...
  it('written content is byte-identical to the bundled example', () => {
    ...
    const written = fs.readFileSync(path.join(projectDir, '.claude', 'agentwright.json'), 'utf8');
  ...
  it('refuses to overwrite existing file without --force', () => {
    ...
    const target = path.join(claudeDir, 'agentwright.json');
  ...
  it('exits non-zero when bundled example is missing', () => {
    const fakePluginRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-fake-plugin-'));
