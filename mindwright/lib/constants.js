...
export const DAEMON_DOWN_WARNING =
  '[mindwright] retrieval daemon is unreachable this turn — recall is ' +
  'degraded for now. The model daemon (re)starts on its own and recall ' +
  'resumes automatically within a minute or two; no restart needed. If it ' +
  'persists, run `/mindwright:status`.';

// Short-term row count that triggers the Stop hook's dream nudge.
export const CAP_EXCHANGES = 50;

// Hard ceiling on rows a single drainBatch pass pulls — and therefore the
// volume one /mindwright:dream consolidates at once. The live path drains
// the oldest DEFAULT_DRAIN_PCT (70%) of short-term; with a large seed
// backlog that 70% is effectively the whole corpus, which no single
// consolidator pass can distill. Capping the SELECT (LIMIT) bounds the
// per-pass volume AND stops drainBatch loading the entire short table into
// memory; the seed loop just runs more bounded passes. Below the cap the
// live 70% behavior is unchanged. Override with MINDWRIGHT_DRAIN_MAX_ROWS.
export const DRAIN_MAX_ROWS = (() => {
  const n = Number(process.env.MINDWRIGHT_DRAIN_MAX_ROWS);
...
// global socket, so the multi-GB resident model set loads once per box.
...
// Daemon idle self-exit (frees the resident models); long enough to outlive
// normal think/tool gaps within an active session.
export const MODEL_DAEMON_IDLE_EXIT_MS = 15 * 60 * 1000;

// RSS ceiling for the model daemon. ONNX Runtime does not return CPU-arena
// memory to the OS even on session disposal, so a residual leak or heap
// fragmentation can still ratchet a long-lived daemon upward despite
// enableCpuMemArena:false + disposeModels(). Crossing this triggers a clean
