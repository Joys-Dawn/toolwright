...
import { isSessionLive } from './session-liveness.js';
import { isModelDaemonAlive } from './model-daemon-status.js';
...
export async function handleToolCall(name, args, ctx) {
  ...
  try {
    ...
    process.stderr.write(
      `[mindwright/tools] tool ${name} threw: ${err && err.stack ? err.stack : err}\n`
    );
...
async function statusHandler(_args, ctx) {
  ...
  // Two distinct liveness signals (see session-liveness.js / model-daemon-
  // status.js): a Claude session bound to this project vs. the machine-wide
  // model daemon actually serving. They answer different questions and must
  // not be conflated.
  const session_alive = isSessionLive();
  const model_daemon_alive = await isModelDaemonAlive();
  ...
  // pending_embeds only back-fill when the MODEL daemon is serving — gate on
  // that, not session liveness (a live session with the model daemon down
  // still has stuck embeds). Warn so the count isn't a mystery.
  if (!model_daemon_alive && pending_embeds > 0) {
    warnings.push(
      `${pluralize(pending_embeds, 'embedding')} ${agree(pending_embeds, 'is', 'are')} pending but the model daemon is not serving — ` +
      `they will be back-filled once it is up (it is lazily started when a Claude Code session opens in any project).`,
    );
  }
