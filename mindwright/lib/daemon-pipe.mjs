 ...
 *
 * @param {object} args
 * @param {string} args.pipePath  pipe path to bind (the fixed machine-global socket; scripts/model-daemon.mjs passes modelDaemonSocketPath())
...
export async function startPipeServer({
  ...
  if (!pipePath) {
    throw new Error('startPipeServer: pipePath required');
  }
  const path = pipePath;
