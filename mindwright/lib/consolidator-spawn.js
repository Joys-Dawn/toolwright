...
// isConsolidatorSession below): a consolidator session must not spawn another.
...
// The read side of that sentinel, consumed by the Stop hook's cap-check so it
// never spawns a consolidator from inside one (the orphan-chain failure
// described above). Two signals, either sufficient. PRIMARY: the
// MINDWRIGHT_IS_CONSOLIDATOR='1' env written above — it inherits down the
// detached supervisor chain, so the session sees it on every hook. SECONDARY:
// the role set, for sessions where `assign_role consolidator` was explicit.
