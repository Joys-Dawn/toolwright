...
async function main() {
  ...
  if (process.env.MINDWRIGHT_USE_STUB_MODELS === '1') {
    // Deterministic, dependency-free stubs so tests stay hermetic (no model
    // daemon, no ONNX). STRICTLY test-only — NOT an offline mode: the constant
    // embedding makes every query's cosine ~1.0, so the 0.85 novelty gate
    // suppresses all retrieval after the first, and rerank encodes candidate
    // order, not relevance.
