...
import { SEED_BATCH_BUDGET_BYTES } from '../lib/constants.js';
...
export function describeNextStep({ total, droppedUnderCallingSession, skippedFiles, transcriptRows = 0, moreRemaining = false }) {
  ...
  if (transcriptRows > 0) {
    const base =
      `Consolidate this slice now: run /mindwright:dream with scope="all" ` +
      `(${transcriptRows} transcript row(s) seeded under their original session ids — ` +
      `a default session-scoped dream would skip them), repeating dream passes until ` +
      `short-term is drained.`;
    return moreRemaining
      ? `${base} Then re-run /mindwright:seed-from-repo for the next slice — more ` +
          `transcript history remains.`
      : `${base} All transcript history has now been seeded.`;
  }
  if (total > 0) {
...
async function main() {
  ...
  try {
    ...
    try {
      // Bounded slice per invocation: ingest at most SEED_BATCH_BUDGET_BYTES
      // of transcript content this run so one /mindwright:seed-from-repo +
      // its inline consolidation stays digestible. Offsets make it resumable
      // — stoppedEarly ⇒ more remains, re-run to continue.
      transcriptSummary = await runSeedLoop({
        store,
        maxBytesPerInvocation: SEED_BATCH_BUDGET_BYTES,
      });
    } catch (e) {
  ...
  const moreRemaining = !!transcriptSummary.stoppedEarly;
  const nextStep = describeNextStep({
    total, droppedUnderCallingSession, skippedFiles, transcriptRows, moreRemaining,
  });
