...
```
   ...
   It prints `{short_count, long_count, by_category, by_category_scope, last_consolidation, model_cached, daemon_alive, pending_embeds, poison_embeds, unbound_count, oldest_preference_at, consolidator, warnings}`. `by_category_scope` keys are strings like `'fact/user'`, `'procedural/role:planner'`, `'episodic/project'`. `consolidator` is `{ session_id, handle, first_seen, last_spawn }` when this requester has ever spawned a background consolidator session for this project, otherwise `null`.
