---
name: setup
description: One-time download of the bge-m3 embedder and gte-reranker-modernbert-base cross-encoder into the local Hugging Face cache, plus a smoke test. Run once after installing mindwright before any memory features become active. First run is ~5-15 min depending on connection.
...
3. Loads the reranker `Alibaba-NLP/gte-reranker-modernbert-base` (raw-logit output — mindwright applies the sigmoid in code).
